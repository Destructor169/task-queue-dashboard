...
[![Build and Publish Docker Images](https://github.com/Destructor169/task-queue-dashboard/actions/workflows/docker-publish.yml/badge.svg)](https://github.com/Destructor169/task-queue-dashboard/actions/workflows/docker-publish.yml)
...
```bash
git clone https://github.com/Destructor169/task-queue-dashboard.git
cd task-queue-dashboard
...
docker run -d --name taskqueue-backend \
  ...
  --link redis \
  taskqueue-backend:latest

# Start frontend
docker run -d --name taskqueue-frontend \
  -p 3000:3000 \
  taskqueue-frontend:latest
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: backend
          image: taskqueue-backend:latest
          ports:
            - containerPort: 8080
          env:
            - name: REDIS_ADDR
